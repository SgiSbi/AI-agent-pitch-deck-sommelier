...
from .interfaces import ISearchClient


class TavilySearchClient(ISearchClient):
    """
    Полноценная реализация ISearchClient (перенос логики из tavily_client.py).
    """
    ...
    def _format_results(self, results: List[dict], max_total_chars: int = 12000) -> str:
        if not results:
            return ""
    ...
    def search(self, queries: List[str], max_results: int = 4) -> str:
        return self._search_internal(queries, max_results_per_query=max_results, return_raw=False)  # type: ignore[return-value]
