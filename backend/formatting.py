#!/usr/bin/env python3
"""
Скрипт для конвертации Markdown (.md) в DOCX.
Поддерживает всё форматирование + ссылки с отображением домена.
"""
...
def process_text_with_formatting(text):
    """
    Обрабатывает текст с Markdown форматированием.
    Возвращает список кортежей (тип, содержимое, url)
    """
    if not text or text == '---':
    ...
    while i < length:
        ...
        while i < length:
            ...
            if (current_char == '*' or 
                current_char == '[' or 
                (i + 1 < length and text[i:i+2] == '**') or 
                (i + 7 < length and text[i:i+7] == 'http://') or
                (i + 8 < length and text[i:i+8] == 'https://')):
                break
            i += 1
...
def add_formatted_text_to_paragraph(paragraph, text):
    ...
    # Проверяем, не является ли текст строкой разделителя таблицы
    if text == '---' or (text.startswith(':---') and text.endswith('---:')):
        # Это строка выравнивания таблицы, пропускаем её
        return
    ...
    for part_type, content, url in parts:
        if part_type == 'text':
            if content and content != '---':  # Не добавляем разделители
...
def convert_md_to_docx(input_file, output_file):
    ...
    try:
        ...
        while i < len(lines):
            ...
            # Проверяем начало таблицы (строка с | и следующая строка тоже с |)
            if '|' in line and i + 1 < len(lines) and '|' in lines[i + 1]:
                ...
                if len(table_lines) >= 2:
                    ...
                    for table_line in table_lines:
                        # Разбиваем по | и убираем пустые ячейки по краям
                    ...
                    # Находим строку заголовков (первая строка)
                    # и пропускаем строку с выравниванием (---)
                    ...
                    for row in rows[1:]:
                        ...
                        if row and all(re.match(r'^:?-{3,}:?$', cell) for cell in row):
                            # Это строка выравнивания, пропускаем
                            continue
                        else:
            ...
            # Горизонтальная линия (вне таблицы)
